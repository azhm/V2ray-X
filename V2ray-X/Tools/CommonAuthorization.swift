...
class CommonAuthorization: NSObject {
    ...
    static let systemProxyAuthRightName = "com.nadaflash.v2ray-x.rights"
            ...
            [
                "system_proxy_set":
                    [AuthorizationRightKey.rightName : CommonAuthorization.systemProxyAuthRightName,
                     ...
                     AuthorizationRightKey.rightDescription : "V2ray-X App want to change system proxy settings."]
        ]
        return sCommandInfo
    }
    ...
    func setupAuthorizationRights(authRef: AuthorizationRef) -> Void {
        ...
        self.enumerateAuthorizationRights(right: {
            ...
            log.info("AuthorizationRightGet with: right name[\(rightName)], result[\(status)]")
            ...
            if (status == errAuthorizationDenied) {
                ...
                log.info("AuthorizationRightSet set Policy Database with: right name[\(rightName)], result[\(status)]")
            }
            ...
            if (status != errAuthorizationSuccess) {
                ...
                log.info(errorMessage ?? "Error adding authorization right: \(rightName)")
            }
        })
    }
